...
// Check if trans_id is provided in the URL
if(isset($_GET['trans_id'])) {
    // Sanitize the ID input to prevent SQL injection
    $trans_id = mysqli_real_escape_string($conn, $_GET['trans_id']);

    // Query to retrieve data for the selected trans_id
    $query = "SELECT * FROM reservations WHERE trans_id = '$trans_id'";
    ...
    if (!$row) {
        die("Error: No data found for the specified transaction ID."); // Display error message and stop script execution
    }
} else {
    // If no trans_id is provided, redirect to the previous page or display an error message
